# Output Template: SG COE Bidding Results

Use this structure unless user requests a different layout.

```md
As of: <YYYY-MM-DD HH:mm SGT>
Scope: <latest exercise / specific exercise / trend window>

Exercise Summary:
- Exercise: <identifier or period>
- Update time: <absolute timestamp>
- Confidence: <High|Medium|Low>
- Sources: <https://...>

Category Results (A-E):
| Category | Quota Premium (S$) | Quota | Bids Received | Success Rate | Change vs Previous |
|---|---:|---:|---:|---:|---:|
| A | <value or N/A> | <...> | <...> | <...> | <+/-value or N/A> |
| B | ... |
| C | ... |
| D | ... |
| E | ... |

Key Changes:
- <largest movement and affected category>
- <notable quota or demand shift>
- Confidence: <High|Medium|Low>
- Sources: <https://...>

Facts vs inference:
- Facts: <published COE exercise data>
- Inference (optional): <clearly labeled interpretation>

Caveats:
- <missing fields, stale source, or verification pending notes>
```
